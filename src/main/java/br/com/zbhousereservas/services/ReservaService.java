...
public class ReservaService {
    ...
    public List<LocalDate> listarDatasDisponiveis() {
        ...
        List<Reserva> reservasFuturas = this.reservaRepository.buscarReservasFuturas(hoje);

        for (Reserva reserva : reservasFuturas) {
            datasReservadas.addAll(validarObjetos.intervaloCheckinChekout(reserva.getCheckin(), reserva.getCheckout()));
        }
...
//            ValidarObjetos.validarValorParcela(valorReserva, valorParcela);
    ...
    public Reserva salvarReserva(ReservaDTO dto) {

        Reserva reserva = Objects.requireNonNull(Reserva.fromEntityReserva(dto)).build();

        Pagamento primeiroPagamento = new Pagamento(dto.pagamentos().get(0));

        reserva.adicionarPagamento(primeiroPagamento);
